function [A,B,C,phi,it1,it2,phi_vec]=bcdLL1_alsls(X,R,L,lsearch,comp,Tol1,MaxIt1,Tol2,MaxIt2,Ninit,A,B,C)
...
if nargin<3;error('At least 3 input arguments are required');end
...
if exist('A')==1 && isempty(A)~=1
    if (size(A,1)==size(X,1)) && (size(A,2)==R*L)
    init_vec(1)=1; 
    A_init=A;
    else
...
if exist('B')==1 && isempty(B)~=1
    if (size(B,1)==size(X,2)) && (size(B,2)==R*L)
    init_vec(2)=1;
    B_init=B;
    else
...
RL=R*L;                    % Number of columns of A and B
L_vec=repmat(L,1,R);       % Pattern of the blocks: L columns for each matrix
...
if strcmp(comp,'on')==1
   ...
   size_core=[I,J,K];
   if (K*J>RL+1) && (I>RL); size_core(1)=RL+1; end   % first dimension can be reduced
   if (K*I>RL+1) && (J>RL); size_core(2)=RL+1; end   % second dimension can be reduced
...
for ninit=1:Ninit   % Try max_init different initializations
    ...
    if init_given==0
        if ninit==1
        [A,B,C]= bcdLL1_init(X,R,L,'gevd');  
        else
        [A,B,C]= bcdLL1_init(X,R,L,'random');  % force random init
        end
    ...
    while stop==0
        ...
        [A,B,C] = bcdLL1_lsearch(A2,B2,C2,dA,dB,dC,X1,lsearch,it1,L_vec); 
...
if strcmp(comp,'on')==1
    ...
    B=U2*B;
    C=U3*C;
    [A,B,C,phi,it2]=bcdLL1_alsls(X_input,R,L,lsearch,'off',Tol2,MaxIt2,[],[],1,A,B,C);
end
end
...
%*******************************************************************************
function [A,B,C] = bcdLL1_lsearch(A,B,C,dA,dB,dC,X,lsearch,it,L_vec)
%BCDLL1_lsearch Line Search for BCD-(L,L,1)
%   New loading matrices A (IxRL), B (JxRL), C(KxR) are computed
...
%      C <- C + mu * dC
%
%   Line Search for bcdLL1 can for instance be used in gradient-based
...
%      - L_vec: the way A and B are partitioned, here L_vec=[L,L,...,L]
%
%   OUTPUTS: 
%
