function [A,B,C,phi,it1,it2,phi_vec]=bcdLM_alsls(X,R,L,M,lsearch,comp,Tol1,MaxIt1,Tol2,MaxIt2,Ninit,A,B,C)
...
for ninit=1:Ninit   % Try max_init different initializations
    ...
    if init_given==0
       [A_cell,B_cell,C_cell]= bcdLM_init(X,L_vec,M_vec);   % random initialization in cell format
       A=cell2mat(A_cell);
       B=cell2mat(B_cell);             
    ...
    elseif init_given==1 
       if sum(init_vec)==1     % only one matrix was provided
            [A_cell,B_cell,C_cell]= bcdLM_init(X,L_vec,M_vec);  % generate random initializations and then overwrite by the one given in input
    ...
    while stop==0
        ...
        [A,B,C] = bcdLM_lsearch(A2,B2,C2,dA,dB,dC,X2,lsearch,it1,L_vec,M_vec); 
...
if strcmp(comp,'on')==1
    [A_cell,B_cell,C_cell,phi,it2]=bcdLM_alsls(X_input,R,L,M,lsearch,'off',Tol2,MaxIt2,[],[],1,A_cell,B_cell,C_cell);
end
A=A_cell;
B=B_cell;
C=C_cell;
...
function [A_cell,A,C_cell]=update_A(C_cell,B_cell,X3,L_vec) 
 ...
 CB_cell=cell(1,R);
 for r=1:R
     CB_cell{r}=tens2mat(tmprod(C_cell{r},B_cell{r},2),3);   % set of R matrices of size KJxL
 end
...
CA_cell=cell(1,R);
for r=1:R
    CA_cell{r}=tens2mat(tmprod(C_cell{r},A_cell{r},1),1);   % set of R matrices of size IKxM
end
...
%*******************************************************************************
function [A,B,C] = bcdLM_lsearch(A,B,C,dA,dB,dC,X,lsearch,it,L_vec,M_vec)
%BCDLM_lsearch Line Search for BCD-(L,M,.)
