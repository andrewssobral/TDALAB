...
R=length(L_vec);
RL=sum(L_vec);                    % Number of columns of A 
RM=sum(M_vec);                    % Number of columns of B
RN=sum(N_vec);
RMN=sum(M_vec.*N_vec);
RLM=sum(L_vec.*M_vec);
RLN=sum(L_vec.*N_vec);
RLMN=sum(L_vec.*M_vec.*N_vec);
...
if strcmp(comp,'on')==1
   ...
   size_core=[I,J,K];
    if min(K*J,I)>=min(RL,RMN); size_core(1)=min(RL,RMN); end        
    if min(I*K,J)>=min(RM,RLN); size_core(2)=min(RM,RLN); end        
    if min(J*I,K)>=min(RN,RLM); size_core(3)=min(RN,RLM); end     
...
for ninit=1:Ninit   % Try max_init different initializations
    ...
    if init_given==0
       [A_cell,B_cell,C_cell,D_cell]= bcdLrMrNr_init(X,L_vec,M_vec,N_vec);   % random initialization in cell format
    ...
    elseif init_given==1 
        if (sum(init_vec)==1) || (sum(init_vec)==2)     % only one or two matrices were provided
            [A_cell,B_cell,C_cell,D_cell]= bcd_LrMrNr_init(X,L_vec,M_vec,N_vec);  % generate random initializations and then overwrite by the one given in input
            if init_vec(1)==1   % A was provided
               A_cell = A_init; 
               [B_cell,B,D_cell]=update_B(A_cell,C_cell,D_cell,X1,M_vec);
               [C_cell,C]=update_C(A_cell,B_cell,D_cell,X2,N_vec);               
               [D_cell]=update_D(A_cell,B_cell,C_cell,X_vec,L_vec,M_vec,N_vec);
            elseif init_vec(2)==1  % B was provided
                B_cell = B_init; 
                [A_cell,A,D_cell]=update_A(B_cell,C_cell,D_cell,X3,L_vec);
                [C_cell,C]=update_C(A_cell,B_cell,D_cell,X2,N_vec); 
                [D_cell]=update_D(A_cell,B_cell,C_cell,X_vec,L_vec,M_vec,N_vec);
            elseif init_vec(3)==1   % C was provided
                C_cell = C_init; 
                [A_cell,A,D_cell]=update_A(B_cell,C_cell,D_cell,X3,L_vec);
                [B_cell,B]=update_B(A_cell,C_cell,D_cell,X1,M_vec);
                [D_cell]=update_D(A_cell,B_cell,C_cell,X_vec,L_vec,M_vec,N_vec);
            elseif init_vec(4)==1   % D was provided
                D_cell = D_init; 
                [A_cell,A,D_cell]=update_A(B_cell,C_cell,D_cell,X3,L_vec);
                [B_cell,B,D_cell]=update_B(A_cell,C_cell,D_cell,X1,M_vec);
                [C_cell,C,D_cell]=update_C(A_cell,B_cell,D_cell,X2,N_vec); 
            end
        elseif sum(init_vec)==3  % three matrices were provided
            ...
            elseif init_vec(2)==0      % A,C and D are given in input and B=[]
               ...
               [B_cell,B,D_cell]=update_B(A_cell,C_cell,D_cell,X1,M_vec);   % so we update B to exploit A and C
               [A_cell,A,D_cell]=update_A(B_cell,C_cell,D_cell,X3,L_vec); 
               [C_cell,C]=update_C(A_cell,B_cell,D_cell,X2,N_vec);  
               [D_cell]=update_D(A_cell,B_cell,C_cell,X_vec,L_vec,M_vec,N_vec);
            ...
            elseif init_vec(3)==0  % A, B and D were given 
               ...
               [C_cell,C,D_cell]=update_C(A_cell,B_cell,D_cell,X2,N_vec);   % so we update C first to exploit A and B
               [A_cell,A,D_cell]=update_A(B_cell,C_cell,D_cell,X3,L_vec); 
               [B_cell,B]=update_B(A_cell,C_cell,D_cell,X1,M_vec);
               [D_cell]=update_D(A_cell,B_cell,C_cell,X_vec,L_vec,M_vec,N_vec);
    ...
    X_est=zeros(I,J,K);
    for r=1:R
        X_est=X_est+tmprod(tmprod(tmprod(D_cell{r},A_cell{r},1),B_cell{r},2),C_cell{r},3);
    end
    phi=norm(X_est(:)-X(:),'fro');
    ...
    while stop==0
        ...
        [A_cell,B_cell,C_cell] = bcdLrMrNr_lsearch(A2_cell,B2_cell,C2_cell,dA_cell,dB_cell,dC_cell,D_cell,X2,lsearch,it1,L_vec,M_vec,N_vec); 
        % Perform ALS updates
        % Update A 
        [A_cell,A,D_cell]=update_A(B_cell,C_cell,D_cell,X3,L_vec);
        % Update  B 
...
if strcmp(comp,'on')==1
    [A_cell,B_cell,C_cell,D_cell,phi,it2]=bcdLrMrNr_alsls(X_input,L_vec,M_vec,N_vec,lsearch,'off',Tol2,MaxIt2,[],[],1,A_cell,B_cell,C_cell,D_cell);
end
A=A_cell;
B=B_cell;
C=C_cell;
...
%*******************************************************************************    
function [A_cell,A,D_cell]=update_A(B_cell,C_cell,D_cell,X3,L_vec)
 % Least Squares Update of A, given B, C and D
 R=length(L_vec);
 Z=cell(1,R);
 for r=1:R
    Z{r}=tens2mat(tmprod(tmprod(D_cell{r},B_cell{r},2),C_cell{r},3),3);   % set of R matrices of size KJxLr
 end
 warning('off')
 A=X3.'/cell2mat(Z).';
 warning('on')
 ...
 R_cell=cell(1,R);
 for r=1:R
    [A_cell{r} R_cell{r}]=qr(A_cell{r},0);
 end
 A=cell2mat(A_cell);
 if nargout==3  % Incorporate the R_cell matrices in D_cell
     for r=1:R
         [Lr Mr,Nr]=size(D_cell{r});
         D_cell{r}=mat2tens(tens2mat(D_cell{r},3)*R_cell{r}.',3,[Lr,Mr,Nr]);
     end
 end
end

%*******************************************************************************
function [B_cell,B,D_cell]=update_B(A_cell,C_cell,D_cell,X1,M_vec)
% Least Squares Update of B, given A, C and D
 R=length(M_vec);
 Z=cell(1,R);
 for r=1:R
    Z{r}=tens2mat(tmprod(tmprod(D_cell{r},A_cell{r},1),C_cell{r},3),1);   
 end
 warning('off')
 B=X1.'/cell2mat(Z).';
 warning('on')
 ...
 R_cell=cell(1,R);
 for r=1:R
    [B_cell{r} R_cell{r}]=qr(B_cell{r},0);
 end
 B=cell2mat(B_cell);
 if nargout==3  % Incorporate the R_cell matrices in D_cell
     for r=1:R
         [Lr Mr,Nr]=size(D_cell{r});
         D_cell{r}=mat2tens(tens2mat(D_cell{r},1)*R_cell{r}.',1,[Lr,Mr,Nr]);
     end
 end
end
     
%*******************************************************************************    
function [C_cell,C,D_cell]=update_C(A_cell,B_cell,D_cell,X2,N_vec)
 % Least Squares Update of C, given A, B and D
 R=length(N_vec);
 Z=cell(1,R);
 for r=1:R
    Z{r}=tens2mat(tmprod(tmprod(D_cell{r},A_cell{r},1),B_cell{r},2),2);   
 end
 warning('off')
 C=X2.'/cell2mat(Z).';
 warning('on')
 ...
 R_cell=cell(1,R);
 for r=1:R
    [C_cell{r} R_cell{r}]=qr(C_cell{r},0);
 end
 C=cell2mat(C_cell);
 if nargout==3  % Incorporate the R_cell matrices in D_cell
     for r=1:R
         [Lr Mr,Nr]=size(D_cell{r});
         D_cell{r}=mat2tens(tens2mat(D_cell{r},2)*R_cell{r}.',2,[Lr,Mr,Nr]);
     end
 end
end
