% demo script to test bcdLrMrNr_alsls algorithm.
...
%-------------------------------------------------------------------------------
% Compute the BCD-(Lr,Mr,Nr) for several initializations
%-------------------------------------------------------------------------------
for ninit=1:Ninit
    disp('-----------------------------------------------------')
    ...
    [A_init,B_init,C_init,D_init]= bcdLrMrNr_init(X,L_vec,M_vec,N_vec);
    %-----------------------------------------------------
    %  COMPUTE THE DECOMPOSITION for this initialization
    %-----------------------------------------------------
    disp('Algorithm 1: als without line search')
    [A_est,B_est,C_est,D_est,phi,it1,it2,phi_als]=bcdLrMrNr_alsls(X,L_vec,M_vec,N_vec,'none',comp,Tol1,MaxIt1,Tol2,MaxIt2,Ninit,A_init,B_init,C_init);
    ...
    [A_est,B_est,C_est,D_est,phi,it1,it2,phi_lsh]=bcdLrMrNr_alsls(X,L_vec,M_vec,N_vec,'lsh',comp,Tol1,MaxIt1,Tol2,MaxIt2,Ninit,A_init,B_init,C_init);
    [errA]=solve_blockperm(cell2mat(A_est),cell2mat(A),L_vec);
    [errB]=solve_blockperm(cell2mat(B_est),cell2mat(B),M_vec);
    [errC]=solve_blockperm(cell2mat(C_est),cell2mat(C),N_vec);   
    disp(['Final phi :  ',num2str(phi)])
    ...
    [A_est,B_est,C_est,D_est,phi,it1,it2,phi_lsb]=bcdLrMrNr_alsls(X,L_vec,M_vec,N_vec,'lsb',comp,Tol1,MaxIt1,Tol2,MaxIt2,Ninit,A_init,B_init,C_init);
    [errA]=solve_blockperm(cell2mat(A_est),cell2mat(A),L_vec);
    [errB]=solve_blockperm(cell2mat(B_est),cell2mat(B),M_vec);
    [errC]=solve_blockperm(cell2mat(C_est),cell2mat(C),N_vec);
    disp(['Final phi :  ',num2str(phi)])
    ...
    [errB]=solve_blockperm(cell2mat(B_est),cell2mat(B),M_vec);
    [errC]=solve_blockperm(cell2mat(C_est),cell2mat(C),N_vec);
    [A_est,B_est,C_est,D_est,phi,it1,it2,phi_elsr]=bcdLrMrNr_alsls(X,L_vec,M_vec,N_vec,'elsr',comp,Tol1,MaxIt1,Tol2,MaxIt2,Ninit,A_init,B_init,C_init);
    disp(['Final phi :  ',num2str(phi)])
    disp(['It1       :  ',num2str(it1)])
    disp(['It2       :  ',num2str(it2)])
    disp(['Error on A:  ',num2str(errA)])
    ...
    [A_est,B_est,C_est,D_est,phi,it1,it2,phi_elsc]=bcdLrMrNr_alsls(X,L_vec,M_vec,N_vec,'elsc',comp,Tol1,MaxIt1,Tol2,MaxIt2,Ninit,A_init,B_init,C_init);
    [errA]=solve_blockperm(cell2mat(A_est),cell2mat(A),L_vec);
    [errB]=solve_blockperm(cell2mat(B_est),cell2mat(B),M_vec);
    [errC]=solve_blockperm(cell2mat(C_est),cell2mat(C),N_vec);
    disp(['Final phi :  ',num2str(phi)])
