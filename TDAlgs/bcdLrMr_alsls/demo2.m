% demo script to test bcdLrMr_alsls algorithm.
% The purpose of this demo is to show how to exploit several initializations.
% The function bcdLrMr_alsls has 2 kinds of optional input arguments to deal with initialization:
...
%-------------------------------------------------------------------------------
% Compute the BCD-(Lr,Mr,.) with a limited number of iterations for each starting point
...
[A_init,B_init,C_init,phi,itInit]=bcdLrMr_alsls(X,L_vec,M_vec,[],[],[],MaxItInit,[],[],Ninit);
% Then go on with the best starting point
[A_est,B_est,C_est,phi,it1]=bcdLrMr_alsls(X,L_vec,M_vec,[],[],[],[],[],[],[],A_init,B_init,C_init);
